...
Example:
python demo_webcam.py --model-checkpoint "torchscript_resnet101_fp32.pth" --resolution 480 640

torchscript_resnet101_fp32.pth
torchscript_resnet50_fp32.pth
torchscript_resnet101_fp16.pth
...
class Camera:
    def __init__(self, device_id=device_id, width=1280, height=720):
...
class Displayer:
    ...
    def step(self, image):
        ...
        if self.webcam is not None:
            ...
            self.webcam.send(image_web)  # schedule_frame(image_web)
...
class Controller:
    ...
    def __init__(self, model_checkpoint_dir):
        ...
        self.refine_mode_cycle = [
            SamplingUpdater(self, 0),
            ThresholdUpdater(self, 1),
            FullUpdater(self, 2),
        ]
        ...
        self.model_checkpoint_cycle = self.get_model_checkpoint_cycle(
    ...
    def get_model_checkpoint_cycle(self, model_checkpoint_dir):
        ...
        for filename in os.listdir(model_checkpoint_dir):
            if "torchscript_" in filename:
                ...
                models.append(
                    (model_name, os.path.join(model_checkpoint_dir, filename))
                )
    ...
    def render(self):
        ...
        for control in self.controls:
            ...
            control_image[
                control["y"] : control["y"] + control["h"],
                control["x"] : control["x"] + control["w"],
            ] = bg_color
            cv2.putText(
                control_image,
                control["label"],
                (control["x"] + 10, control["y"] + control["h"] // 2),
    ...
    def clicked(self, control):
        if control["name"] == "mode_switch":
            ...
            self.update_backbone_scale(
                round(min(bgmModel.backbone_scale + 0.10, 1.0), 2)
            )
        elif control["name"] == "backbone_scale_-":
            self.update_backbone_scale(
                round(max(bgmModel.backbone_scale - 0.10, 0.0), 2)
            )
        elif control["name"] == "refine_mode":
            next_index = (self.refine_mode_updater.index + 1) % len(
                self.refine_mode_cycle
            )
    ...
    def switch_composition(self, control):
        current_idx = next(
            i for i, v in enumerate(self.compose_cycle) if v[0] == app["compose_mode"]
        )
    ...
    def update_model_checkpoint(self):
        ...
        self.control_index["model_checkpoint"]["label"] = self.model_checkpoint_cycle[
            next_idx
        ][0]
        bgmModel.reload()
    ...
    def _raw_process_click(self, event, x, y, flags, params):
        if event == cv2.EVENT_LBUTTONDOWN:
            for control in self.controls:
                ...
                if (
                    x > control["x"]
                    and x < control["x"] + control["w"]
                    and y > control["y"]
                    and y < control["y"] + control["h"]
                ):
...
class SamplingUpdater(RefineModeUpdater):
    ...
    def update_value_label(self):
        self.controller.control_index["refine_mode_value"]["label"] = str(
            bgmModel.refine_sample_pixels
        )
        bgmModel.reload()

    def update_model_value(self, delta):
        bgmModel.refine_sample_pixels = max(
            bgmModel.refine_sample_pixels + delta * 40000, 0
        )
        self.update_value_label()
...
class ThresholdUpdater(RefineModeUpdater):
    ...
    def update_value_label(self):
        self.controller.control_index["refine_mode_value"]["label"] = str(
            bgmModel.refine_threshold
        )
        bgmModel.reload()

    def update_model_value(self, delta):
        bgmModel.refine_threshold = min(
            max(bgmModel.refine_threshold + delta * 0.01, 0), 1.0
        )
        self.update_value_label()
...
class BGModel:
    ...
    def reload(self):
        self.model = torch.load(self.model_checkpoint)
...
def grab_bgr():
    bgr_frame = cam.read()
    bgr_blur = cv2.GaussianBlur(bgr_frame.astype("float32"), (67, 67), 0).astype(
        "uint8"
...
def app_step():
    if app["mode"] == "background":
        ...
        cudaModel = bgmModel.model.cuda()
        pha, fgr = cudaModel(src, app["bgr"])[:2]
...
if __name__ == "__main__":
    ...
    with pyvirtualcam.Camera(
        ...
        args = load_args()
        source_device_id = 1  # args.source_device_id
        ...
        dsp = Displayer(
            "RTHRBM Preview", cam.width, cam.height, show_info=(not args.hide_fps)
        )
